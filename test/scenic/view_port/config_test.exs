...
#   defstruct name: nil, default_scene: nil, default_scene_activation: nil, drivers: []
  ...
  test "valid? passes a default scene and a name" do
    assert Config.valid?(
      %Config{default_scene: :some_scene, name: :abc, size: {640, 480}} )
    assert Config.valid?(
      %Config{default_scene: {:some_mod, nil}, name: :abc, size: {640, 480}} )
  end
  ...
  test "valid! passes a default scene and a name" do
    assert Config.valid!(
      %Config{default_scene: :some_scene, name: :abc, size: {640, 480}} ) == :ok
    assert Config.valid!(
      %Config{default_scene: {:some_mod, nil}, name: :abc, size: {640, 480}} ) == :ok
  end
