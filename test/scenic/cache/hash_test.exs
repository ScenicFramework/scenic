...
#    hash = Cache.File.compute_file_hash( @valid_hash_path )
  ...
  test "verify_file returns {:ok, data} when the hash checks out ok" do
    ...
    assert Hash.verify_file( {@missing_hash_path, @missing_hash_256, :sha256} ) == {:ok, @missing_hash_256}
  end
  ...
  test "path_params(path, hash) works" do
    assert Hash.path_params( @missing_hash_path, @missing_hash ) == {@missing_hash_path, @missing_hash, :sha}
    assert Hash.path_params( {@missing_hash_path, @missing_hash} ) == {@missing_hash_path, @missing_hash, :sha}
  end
  ...
  test "path_params(path, hash, type) works" do
    assert Hash.path_params( @missing_hash_path, @missing_hash_256, :sha256 )    == {@missing_hash_path, @missing_hash_256, :sha256}
    assert Hash.path_params( {@missing_hash_path, @missing_hash_256, :sha256} )  == {@missing_hash_path, @missing_hash_256, :sha256}
  end
  ...
  test "path_params(path, hash, type) with bogus params fails" do
    ...
    assert_raise FunctionClauseError, fn -> Hash.path_params( {@missing_hash_path, 1232, :sha256} ) end

    assert_raise FunctionClauseError, fn -> Hash.path_params( @missing_hash_path, @missing_hash, 123 ) end
    assert_raise FunctionClauseError, fn -> Hash.path_params( {@missing_hash_path, @missing_hash, 123} ) end
  end
