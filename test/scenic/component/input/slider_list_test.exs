...
defmodule Scenic.Component.Input.SliderListTest do
  ...
  test "press/release moves the slider and sends a message but pressing again in the same spot does not", %{vp: vp, pid: pid} do
    Input.send( vp, @press )
  ...
  test "implements put/fetch", %{scene: scene} do
    ...
    assert Component.fetch( pid ) == { :ok, :b }
    assert Component.put( pid, :a ) == :ok
    assert Component.fetch( pid ) == { :ok, :a }
    assert Component.put( pid, :invalid ) == {:error, :invalid}
    assert Component.fetch( pid ) == { :ok, :a }
  end
