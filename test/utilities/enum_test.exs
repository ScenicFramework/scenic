defmodule Scenic.Utilities.EnumTest do
  ...
  test "filter_reduce filters a list (in the right order) and reduces" do
    {even, odd} =
      Scenic.Utilities.Enum.filter_reduce(@test_list, [], fn e, acc ->
        case rem(e, 2) do
          0 -> {true, acc}
          _ -> {false, [e | acc]}
        end
  ...
  test "filter_reduce filters and reduces a map" do
    {even, odd} =
      Scenic.Utilities.Enum.filter_reduce(@test_map, %{}, fn {k, v}, acc ->
        case rem(v, 2) do
          0 -> {true, acc}
  ...
  test "filter_map_reduce filters and maps and reduces a list (in the right order) in one pass" do
    {filter_mapped, count} =
      Scenic.Utilities.Enum.filter_map_reduce(@test_list, 0, fn e, acc ->
        case rem(e, 2) do
          0 -> {true, e * 2, acc + 1}
          _ -> {false, acc}
        end
  ...
  test "filter_map_reduce filters and maps and reduces a map in one pass" do
    {filter_mapped, count} =
      Scenic.Utilities.Enum.filter_map_reduce(@test_map, 0, fn {k, v}, acc ->
        case rem(v, 2) do
          0 -> {true, {k, v * 2}, acc + 1}
          _ -> {false, acc}
        end
