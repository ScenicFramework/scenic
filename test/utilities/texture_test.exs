...
defmodule Scenic.Utilities.TextureTest do
  ...
  test "build creates a g texture" do
    {:ok, {:g, @width, @height, pixels, [clear: 3]}} = Texture.build( :g, @width, @height, clear: 3 )
    assert byte_size(pixels) == @width * @height
    <<
      3 :: size(8),
      3 :: size(8),
  ...
  test "build creates a g texture with a named color" do
    {:ok, {:g, @width, @height, pixels, [clear: 236]}} = Texture.build( :g, @width, @height, clear: :beige )
    assert byte_size(pixels) == @width * @height
    <<
      236 :: size(8),
      236 :: size(8),
  ...
  test "build creates a ga texture" do
    {:ok, {:ga, @width, @height, pixels, [clear: {3,7}]}}  = Texture.build( :ga, @width, @height, clear: {3,7} )
    assert byte_size(pixels) == @width * @height * 2
    <<
      3 :: size(8), 7 :: size(8),
      3 :: size(8), 7 :: size(8),
  ...
  test "build creates a ga texture from a named clear color" do
    {:ok, {:ga, @width, @height, pixels, [clear: {250,255}]}}  = Texture.build( :ga, @width, @height, clear: :azure )
    assert byte_size(pixels) == @width * @height * 2
    <<
      250 :: size(8), 255 :: size(8),
      250 :: size(8), 255 :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "build creates a rgb texture" do
    {:ok, {:rgb, @width, @height, pixels, [clear: {3,7,11}]}} = Texture.build( :rgb, @width, @height, clear: {3,7,11} )
    assert byte_size(pixels) == @width * @height * 3
    <<
      3 :: size(8), 7 :: size(8), 11 :: size(8),
      3 :: size(8), 7 :: size(8), 11 :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "build creates a rgb texture with a named clear color" do
    {:ok, {:rgb, @width, @height, pixels, [clear: {189,183,107}]}} = Texture.build( :rgb, @width, @height, clear: :dark_khaki )
    assert byte_size(pixels) == @width * @height * 3
    <<
      189 :: size(8), 183 :: size(8), 107 :: size(8),
      189 :: size(8), 183 :: size(8), 107 :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "build creates a rgba texture with black as default clear " do
    ...
    <<
      0 :: size(8), 0 :: size(8), 0 :: size(8), 0xff :: size(8),
      0 :: size(8), 0 :: size(8), 0 :: size(8), 0xff :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "build creates a rgba texture" do
    {:ok, {:rgba, @width, @height, pixels, [clear: {3,7,11,17}]}} = Texture.build( :rgba, @width, @height, clear: {3,7,11,17} )
    assert byte_size(pixels) == @width * @height * 4
    <<
      3 :: size(8), 7 :: size(8), 11 :: size(8), 17 :: size(8),
      3 :: size(8), 7 :: size(8), 11 :: size(8), 17 :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "build creates a rgba texture with a named clear color" do
    {:ok, {:rgba, @width, @height, pixels, [clear: {0xF5, 0xFF, 0xFA, 0xFF}]}} = Texture.build( :rgba, @width, @height, clear: :mint_cream )
    assert byte_size(pixels) == @width * @height * 4
    <<
      0xF5 :: size(8), 0xFF :: size(8), 0xFA :: size(8), 0xFF :: size(8),
      0xF5 :: size(8), 0xFF :: size(8), 0xFA :: size(8), 0xFF :: size(8),
      _ :: binary
    >> = pixels
  end
  ...
  test "get and put work with rgba textures" do
    ...
    {:rgba, @width, @height, _, [clear: {3,7,11,13}]} = tex = Texture.put!( tex, 1, 2, {4,8,12,14}  )
    assert Texture.get( tex, 1, 1 ) == {3,7,11,13} 
  ...
  test "clear! works with g textures uses black by default" do
    tex = Texture.build!( :g, @width, @height )
    |> Texture.put!( 1, 1, 7 )
    |> Texture.put!( 1, 2, 9 )
  ...
  test "clear! works with g textures uses texture clear by default" do
    tex = Texture.build!( :g, @width, @height, clear: :dark_khaki )
    |> Texture.put!( 1, 1, 7 )
    |> Texture.put!( 1, 2, 9 )
    assert Texture.get( tex, 1, 1 ) == 7 
    assert Texture.get( tex, 1, 2 ) == 9
  ...
  test "clear! works with ga textures uses black by default" do
    tex = Texture.build!( :ga, @width, @height )
    |> Texture.put!( 1, 1, {3,7} )
    |> Texture.put!( 1, 2, {4,8} )
    assert Texture.get( tex, 1, 1 ) == {3,7} 
    assert Texture.get( tex, 1, 2 ) == {4,8} 
  ...
  test "clear! works with ga textures uses texture clear by default" do
    tex = Texture.build!( :ga, @width, @height, clear: :dark_khaki )
    |> Texture.put!( 1, 1, {3,7} )
    |> Texture.put!( 1, 2, {4,8} )
    assert Texture.get( tex, 1, 1 ) == {3,7} 
    assert Texture.get( tex, 1, 2 ) == {4,8} 
  ...
  test "clear! works with rgb textures uses black by default" do
    tex = Texture.build!( :rgb, @width, @height )
  ...
  test "clear! works with rgb textures uses texture clear by default" do
    tex = Texture.build!( :rgb, @width, @height, clear: :dark_khaki  )
    |> Texture.put!( 1, 1, {3,7,11} )
    |> Texture.put!( 1, 2, {4,8,12} )
    assert Texture.get( tex, 1, 1 ) == {3,7,11} 
    assert Texture.get( tex, 1, 2 ) == {4,8,12}
  ...
  test "clear! works with rgba textures uses black by default" do
    tex = Texture.build!( :rgba, @width, @height )
  ...
  test "clear! works with rgba textures uses texture clear by default" do
    tex = Texture.build!( :rgba, @width, @height, clear: :dark_khaki )
    |> Texture.put!( 1, 1, {3,7,11,13} )
    |> Texture.put!( 1, 2, {4,8,12,14} )
    assert Texture.get( tex, 1, 1 ) == {3,7,11,13}
    assert Texture.get( tex, 1, 2 ) == {4,8,12,14}
