...
overal draw order. When the Scene decides the graph is ready to be drawn to the
...
documentaion](https://elixir-lang.org/getting-started/module-attributes.html).
...
The second parameter, `opts` is a Keword list of contextual/optional data that
...
First, it triggers the [lifecycle](scene_lifecycle.html) of any components the
...
and verifies that it is correctly formatted. If it is correnct, return
...
will trigger the [lifecycle](scene_lifecycle.html) management of the
...
Next, you should read about the [lifecycle of a scene](scene_lifecycle.html).
