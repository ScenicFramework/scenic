...
#  end
  ...
  def get(matrix, x, y)
  def get(matrix, x, y) when is_integer(x) and is_integer(y) and
      x >= 0 and y >= 0 and x < 4 and y < 4 do
    skip_size = (y * 4 * 4) + (x * 4)
    <<
  ...
  def put(matrix, x, y, v) when is_integer(x) and is_integer(y) and is_float(v) and
      x >= 0 and y >= 0 and x < 4 and y < 4 do
    skip_size = (y * 4 * 4) + (x * 4)
    <<
  ...
  def close?(  << _ :: binary-size(@matrix_size)>> = a,
  ...
  def mul( matrix_list ) when is_list(matrix_list) do
    nif_multiply_list( matrix_list )    # in NIF
  end
  ...
  def mul( a, s ) when is_float(s) do
    nif_multiply_scalar( a, s )    # in NIF
  end
  ...
  def project_vector( a, {x, y} ) do
    nif_project_vector2( a, x, y )    # in NIF
  end
  #--------------------------------------------------------
