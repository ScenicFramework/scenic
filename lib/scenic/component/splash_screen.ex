defmodule Scenic.SplashScreen do
  ...
  def init( {initial_scene, args, opts} ) do
    state = %{
      ...
      timer: :timer.send_interval(@splash_interval, :splash_interval),
...
#    animations: animations,
    ...
    if elapsed_time >= min_time do
      case initial_scene do
        {mod, init_data} ->
IO.puts "SPLASH done. Starting dynamic scene #{inspect({mod, init_data})}"
          # dynamic scene. Can just start it up
          ViewPort.set_root( {mod, init_data}, args )

        name when is_atom(name) ->
IO.puts "SPLASH done. Starting managed scene #{inspect(name)}"
