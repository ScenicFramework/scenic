defmodule Scenic.Component.Icon do
  @moduledoc """
  ...
  * `:button_font_size` - the size of the font in the button
    :img - pass a hashed image file for the background
  ...
  * `:active` - the background while the button is pressed
  ...
  import Scenic.Primitives, only: [{:rrect, 3}, {:text, 3}, {:rect, 3}, {:update_opts, 2}]
  ...
  def info(data) do
    """
    #{IO.ANSI.red()}Icon data must be a bitstring: initial_text
