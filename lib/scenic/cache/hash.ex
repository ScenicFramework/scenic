...
#  import IEx
  ...
  def compute_file!( path, hash_type ) do
    # start the hash context
    hash_context = valid_hash_type!( hash_type )
    |> :crypto.hash_init()
    # stream the file into the hash
  ...
  def path_params( {path_or_data, hash, hash_type} ) when is_binary(path_or_data) and
  is_bitstring(hash) and is_atom(hash_type) do
