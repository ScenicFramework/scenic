...
defmodule Scenic.Script do
  ...
  @type script_op ::
    ...
    {:draw_triangle, {x0::number, y0::number, x1::number, y1::number, x2::number, y2::number, fill_stroke()}} |
    {:script, id::pos_integer} |
    :begin_path | :close_path | :fill_path | :stroke_path |
    {:move_to, {x::number, y::number}} |
    {:line_to, {x::number, y::number}} |
    {:arc_to, {x1::number, y1::number, x2::number, y2::number, radius::number}} |
    {:bezier_to, {cp1x::number, cp1y::number, cp2x::number, cp2y::number, x::number, y::number}} |
    {:quadratic_to, {cpx::number, cpy::number, x::number, y::number}} |
    {:quad, {x0::number, y0::number, x1::number, y1::number, x2::number, y2::number, x3::number, y3::number}} |
    ...
    {:fill_linear, {start_x::number, start_y::number, end_x::number, end_y::number, color_start::Color.t(), color_end::Color.t()}} |
    {:fill_radial, {center_x::number, center_y::number, inner_radius::number, outer_radius::number, color_start::Color.t(), color_end::Color.t()}} |
    {:fill_image, image::Static.id()} |
    {:fill_stream, id::Stream.id()} |
    {:stroke_color, color::Color.t()} |
    {:stroke_linear, {start_x::number, start_y::number, end_x::number, end_y::number, color_start::Color.t(), color_end::Color.t()}} |
    {:stroke_radial, {center_x::number, center_y::number, inner_radius::number, outer_radius::number, color_start::Color.t(), color_end::Color.t()}} |
    {:stroke_image, image::Static.id()} |
    {:stroke_stream, id::Stream.id()} |
    {:stroke_width, width::number} |
    {:cap, :butt} | {:cap, :round} | {:cap, :square} |
    {:join, :bevel} | {:join, :round} | {:join, :miter} |
    ...
    {:text_base, :top} | {:text_base, :middle} | {:text_base, :alphabetic} | {:text_base, :bottom}
  ...
  @spec draw_rounded_rectangle( ops::t(),
  ...
  @spec draw_sector( ops::t(),
    radius::number, radians::number,
    fill_stroke_flags :: fill_stroke()
  ) :: ops::t()
    ...
    [{:fill_linear,
      {
        ...
        Color.to_rgba(color_start), Color.to_rgba(color_end)
      }
    } | ops]
  end

  @spec fill_radial( ops::t(),
  ...
  def fill_image( ops, id ) when is_atom(id) or is_bitstring(id) do
    id = with {:ok, id_str} <- Static.resolve_alias(id),
  ...
  def serialize( script, op_fn, param ) when is_list(script) do
    {io_list, param} = Enum.reduce(script, {[], param}, fn(op, {acc, param}) ->
  ...
  defp serialize_op( {:draw_sprites, {id, cmds}} ) do
    << hash :: binary-size(32) >> = with {:ok, {:image, _}} <- Static.fetch(id),
    ...
    {cmds, count} = Enum.reduce(cmds, {[],0}, fn
  ...
  defp serialize_op( {:stroke_linear,
    {
      start_x, start_y, end_x, end_y,
      start_color, end_color
    }
  ...
  defp serialize_op( {:stroke_radial,
    {
      center_x, center_y, inner_radius, outer_radius,
      start_color, end_color
    }
  ...
  defp serialize_op( {:font, id} ) do
    << hash :: binary-size(32) >> = with {:ok, {:font, _}} <- Static.fetch(id),
    {:ok, bin_hash, _str_hash} <- Static.to_hash(id) do
      bin_hash
    else 
  ...
  defp serialize_op({:text_base, :middle}), do: << @op_text_base::16-big, @baseline_middle::16-big >>
  defp serialize_op({:text_base, :alphabetic}), do: << @op_text_base::16-big, @baseline_alphabetic::16-big >>
  defp serialize_op({:text_base, :bottom}), do: << @op_text_base::16-big, @baseline_bottom::16-big >>
  ...
  defp deserialize_op( << @op_pop_push_state::16-big, 0::16, bin::binary >> ),  do: {:pop_push_state, bin}
  ...
  defp deserialize_op(
    ...
    {cmds, bin} = Enum.reduce(1..count, {[], bin}, fn(_, {cmds, bin}) ->
      <<
        sx::float-32-big,
        sy::float-32-big,
        sw::float-32-big,
    ...
    {
      {
        ...
        {
          start_x, start_y, end_x, end_y,
          {:color_rgba, {sr,sg,sb,sa}}, {:color_rgba, {er,eg,eb,ea}}
        }
      },
      bin
    }
  ...
  defp deserialize_op(
    << @op_text_align::16-big, @align_left::16, bin::binary >>
  ), do: {{:text_align, :left}, bin}
  defp deserialize_op(
    << @op_text_align::16-big, @align_center::16, bin::binary >>
  ), do: {{:text_align, :center}, bin}
  defp deserialize_op(
    << @op_text_align::16-big, @align_right::16, bin::binary >>
  ), do: {{:text_align, :right}, bin}

  defp deserialize_op(
    << @op_text_base::16-big, @baseline_top::16, bin::binary >>
  ), do: {{:text_base, :top}, bin}
  defp deserialize_op(
    << @op_text_base::16-big, @baseline_middle::16, bin::binary >>
  ), do: {{:text_base, :middle}, bin}
  defp deserialize_op(
    << @op_text_base::16-big, @baseline_alphabetic::16, bin::binary >>
  ), do: {{:text_base, :alphabetic}, bin}
  defp deserialize_op(
    << @op_text_base::16-big, @baseline_bottom::16, bin::binary >>
  ), do: {{:text_base, :bottom}, bin}


  defp extract_string(bin, bytes) do
    buff_size = case rem( bytes, 4 ) do
      0 -> 0
      1 -> 3
      2 -> 2
      3 -> 1
