...
#========================================================
  ...
  defp compile_primitive( ops, primitive, primitives, %GraphCompiler{} = state ) do
    ...
    styles = case Map.get(primitive, :styles) do
    ...
    {ops, state} = case compile_prim?( primitive, state.reqs ) do
  ...
  defp do_compile_primitive( ops, primitive, primitives, %GraphCompiler{} = state ) do
    ...
    {prim_ops, style_ops, state} = case tx_ops do
  ...
  defp compile_prim?(%Primitive{module: Primitive.Ellipse}, styles ), do: !!Script.draw_flag(styles)
  defp compile_prim?(%Primitive{module: Primitive.Line}, styles ), do: Script.draw_flag(styles) == :stroke
  defp compile_prim?(%Primitive{module: Primitive.Rectangle}, styles ), do: !!Script.draw_flag(styles)
  defp compile_prim?(%Primitive{module: Primitive.Quad}, styles ), do: !!Script.draw_flag(styles)
  defp compile_prim?(%Primitive{module: Primitive.RoundedRectangle}, styles ), do: !!Script.draw_flag(styles)
  ...
  defp do_primitive( %Primitive{module: Primitive.Group, data: ids}, primitives, state
  ) do
    {ops, state} = Enum.reduce(ids, {[], state}, fn(id, {ops,state}) ->
  ...
  defp do_primitive( %Primitive{module: Primitive.Component, data: {_,_,name}}, _, %{vp: vp} = state ) do
    { do_compile_script_name([], vp, name), [], state }
  end

  defp do_primitive( %Primitive{module: Primitive.Text, data: text}, _, state ) do
    {st_ops, state} = compile_styles(
