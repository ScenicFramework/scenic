...
defmodule Scenic.Math.Vector4 do
  ...
  # --------------------------------------------------------
  # build from values
  def build(x, y, z, w) when is_float(x) and is_float(y) and is_float(z) and is_float(w),
    do: {x, y, z, w}
  ...
  def in_bounds({vx, vy, vz, vw}, {bdx, bdy, bdz, bdw}),
    do:
      {vx, vy, vz, vw} == clamp({vx, vy, vz, vw}, {-bdx, -bdy, -bdz, -bdw}, {bdx, bdy, bdz, bdw})
  ...
  def in_bounds({vx, vy, vz, vw}, {minx, miny, minz, minw}, {maxx, maxy, maxz, maxw}),
    do:
      {vx, vy, vz, vw} ==
        clamp({vx, vy, vz, vw}, {minx, miny, minz, minw}, {maxx, maxy, maxz, maxw})
