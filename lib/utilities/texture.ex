...
defmodule Scenic.Utilities.Texture do
  ...
  def build( :g, width, height, opts ) when
  is_integer(width) and is_integer(height) and width > 0 and height > 0 do
    {g, opts} = case opts[:clear] do
      nil -> {0,opts}
      hint_clear ->
  ...
  def build( :ga, width, height, opts ) when
  is_integer(width) and is_integer(height) and width > 0 and height > 0 do
    {{g,a}, opts} = case opts[:clear] do
      nil -> {{0, 0xff}, opts}
      hint_clear ->
  ...
  def build( :rgb, width, height, opts ) when
  is_integer(width) and is_integer(height) and width > 0 and height > 0 do
    {{r,g,b}, opts} = case opts[:clear] do
      nil -> {{0, 0, 0}, opts}
      hint_clear ->
  ...
  def build( :rgba, width, height, opts ) when
  is_integer(width) and is_integer(height) and width > 0 and height > 0 do
    {{r,g,b,a}, opts} = case opts[:clear] do
      nil -> {{0, 0, 0, 0xff}, opts}
      hint_clear -> 
  ...
  defp prep_color( :rgb, {r,g,b} ) when
  is_integer(r) and is_integer(g) and is_integer(b) do
    {r,g,b}
  end
